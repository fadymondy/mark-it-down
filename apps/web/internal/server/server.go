...
import (
	...
	_ "github.com/fadymondy/mark-it-down/apps/web/internal/admin" // mounts the /api/admin user-management + mail surface
	...
	"github.com/fadymondy/mark-it-down/apps/web/internal/graph/resolvers"
	"github.com/fadymondy/mark-it-down/apps/web/internal/mcpsrv"
	_ "github.com/fadymondy/mark-it-down/apps/web/internal/plugins" // blank-imports installed plugins (togo install)
	"github.com/fadymondy/mark-it-down/apps/web/internal/rest"
	"github.com/fadymondy/mark-it-down/apps/web/internal/updates"
)
...
func Boot() *app.App {
	...
	// Resolve the caller for every request when credentials are present (session
	// cookie or bearer PAT) without rejecting anonymous ones — the generated
	...
	api := humachi.New(k.Router, huma.DefaultConfig("Mark It Down API", "0.3.0"))
	...
	k.Router.Handle("/graphql/play", playground.Handler("Web GraphQL", k.Config.GraphQLPath))

	k.Router.Get("/api/health", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = fmt.Fprintf(w, `{"status":"ok","service":"web","togo":%q}`, togoVersion())
	})
...
func OpenAPI() ([]byte, error) {
	router := chi.NewMux()
	api := humachi.New(router, huma.DefaultConfig("Web API", "0.1.0"))
